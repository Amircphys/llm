...
def train_model(
    ...
    for epoch in trange(num_epochs, desc="Epochs"):
        ...
        for src, trg in tqdm(train_dataloader, desc='Train', leave=False):
            ...
            #print(output.shape, trg[:, 1:].ravel().shape)
        ...
        logger.info(f"train loss: {round(train_loss, 3)}, perplexity: {round(torch.exp(torch.tensor(train_loss)).item(), 3)}")
        ...
        logger.info(f"valid loss: {round(valid_loss, 3)}, perplexity: {round(torch.exp(torch.tensor(valid_loss)).item(), 3)}")
