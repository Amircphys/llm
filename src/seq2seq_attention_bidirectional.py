...
class Encoder(nn.Module):
    def __init__(self, input_dim, emb_dim, enc_hid_dim, dec_hid_dim, n_layers, dropout):
        ...
        self.rnn = nn.GRU(emb_dim, enc_hid_dim, bidirectional = True, num_layers=n_layers)
    ...
    def forward(self, src):
        ...
        return outputs, torch.concat([hidden[:-2].mean(axis=0).unsqueeze(0), hidden[-2:].mean(axis=0).unsqueeze(0)], axis=0)
...
class Decoder(nn.Module):
    def __init__(self, output_dim, emb_dim, enc_hid_dim, dec_hid_dim, n_layers, dropout, attention):
        ...
        self.rnn = nn.GRU((enc_hid_dim * 2) + emb_dim, dec_hid_dim, num_layers=n_layers)
